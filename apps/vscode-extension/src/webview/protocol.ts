 ...
 * Theme Color registry and drives search/category browsing; the webview
 * owns rendering and resolution, and reports resolved values back so the
 * host can build `workbench.colorCustomizations` JSON or copy an id to the
 * clipboard.
 */
...
export type HostToWebviewMessage =
  | { readonly type: 'categories'; readonly categories: readonly string[] }
  | { readonly type: 'searchResults'; readonly results: readonly ThemeColorSearchResult[] };

export type WebviewToHostMessage =
  | { readonly type: 'ready' }
  | { readonly type: 'search'; readonly query: string }
  | { readonly type: 'browseCategory'; readonly category: string }
